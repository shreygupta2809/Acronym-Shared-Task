...
def predict(data):
    predictions = []
    for d in data:
        pred = {"id": d["id"], "predictions": ["O"] * len(d["tokens"])}
        for i, t in enumerate(d["tokens"]):
            if (
                ...
                if i < len(d["tokens"]) - 3 and d["tokens"][i + 1] == "(":
                    left = False
                    for j in range(
                        i + 2, min([i + 2 + long_cand_length, len(d["tokens"])])
                    ):
                        if d["tokens"][j] != ")":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-input", type=str, help="Path the the input file (e.g., dev.json)"
    )
    parser.add_argument("-output", type=str, help="Prediction file path")
